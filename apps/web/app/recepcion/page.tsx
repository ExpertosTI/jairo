...
export default function RecepcionCommandCenter() {
    ...
    // Sincronización Maestra con el Stack
    const syncAttendance = useCallback(async () => {
        ...
        } catch (e) { console.warn("Stack Offline - Reintentando..."); }
    }, []);

    useEffect(() => {
        syncAttendance();
        const interval = setInterval(syncAttendance, 10000); // Auto-sync cada 10s
        return () => clearInterval(interval);
    }, [syncAttendance]);

    // Inteligencia de Datos (Detección de Empresa)
    useEffect(() => {
            ...
            <nav className="h-20 border-b border-white/5 bg-[#020408]/95 backdrop-blur-3xl flex items-center justify-between px-6 lg:px-12 z-50 sticky top-0 shadow-2xl">
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-emerald-600 rounded-xl flex items-center justify-center border border-emerald-400/20 shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                            <Fingerprint className="w-6 h-6" />
                        </div>
                        <div className="hidden sm:block text-left">
                            <span className="text-xl font-black uppercase tracking-tighter italic">JairoOS</span>
                            <span className="block text-[8px] font-black text-emerald-500 uppercase tracking-widest mt-0.5">SINC_ACTIVA: {status === 'error' ? 'OFFLINE' : 'CONECTADO'}</span>
                        </div>
                    </div>
                ...
                <div className="flex gap-2">
                    <button onClick={() => setView('directory')} className={`p-3.5 rounded-2xl transition-all ${view === 'directory' ? 'bg-emerald-600 shadow-lg text-white' : 'bg-white/5 text-gray-600 hover:text-white'}`}><LayoutGrid className="w-5 h-5" /></button>
                    <button onClick={() => setView('tables')} className={`p-3.5 rounded-2xl transition-all ${view === 'tables' ? 'bg-emerald-600 shadow-lg text-white' : 'bg-white/5 text-gray-600 hover:text-white'}`}><Layers className="w-5 h-5" /></button>
                </div>
            </nav>
                ...
                {view === 'directory' ? (
                        ...
                        <AnimatePresence>
                            {filtered.map((inv) => (
                                <motion.div 
                                    ...
                                    className={`p-8 rounded-[2.5rem] border cursor-pointer transition-all bg-white/[0.01] relative group active:scale-95 ${inv.status === 'cleared' ? 'border-emerald-500/40 bg-emerald-500/[0.05] shadow-2xl' : 'border-white/5 hover:border-emerald-500/30 hover:bg-white/[0.03]'}`}
                                >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
                        {mesasIds.map(mesaId => {
                            ...
                            return (
                                <div key={mesaId} className="p-10 rounded-[3.5rem] bg-white/[0.01] border border-white/5 hover:border-white/10 transition-all group relative overflow-hidden flex flex-col h-full shadow-lg">
                ...
                {selectedGuest && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 backdrop-blur-3xl bg-black/95">
                        <motion.div 
                            ...
                            className="w-full max-w-[950px] max-h-[95vh] bg-[#05080f] rounded-[4rem] border border-white/10 flex flex-col relative overflow-hidden shadow-[0_50px_150px_rgba(0,0,0,1)]"
                        >
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar">
                                <div className="p-12 lg:p-16 border-b border-white/5 bg-white/[0.02] relative">
                                    ...
                                    <div className="group relative">
                                        ...
                                        <input 
                                            ...
                                            className="text-5xl lg:text-7xl bg-transparent font-black uppercase leading-[0.85] tracking-tighter outline-none w-full border-b border-white/10 focus:border-emerald-500/50 pb-4 transition-all"
                                            placeholder="CORREGIR NOMBRE"
                                        />
                                    </div>
                                    
                                    <div className="mt-8 flex items-center gap-4">
                                        <p className={`text-2xl font-bold uppercase tracking-widest italic ${EMPRESAS_DOMINIOS[formData.correo.split("@")[1]?.toLowerCase()] ? 'text-emerald-500' : 'text-gray-600'}`}>
                                ...
                                <div className="p-10 lg:p-14 space-y-10">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="relative group">
                                                    ...
                                                    <input 
                                                        ...
                                                        className="w-full h-20 bg-white/[0.03] border border-white/10 rounded-2xl pl-16 pr-8 font-black text-xl outline-none focus:border-emerald-500/50 shadow-inner"
                                                    />
                                                </div>
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[11px] font-black text-gray-500 uppercase tracking-widest">Empresa / Origen</label>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <select 
                                                        ...
                                                        className="w-full h-20 bg-white/[0.03] border border-white/10 rounded-2xl pl-16 pr-8 font-black text-xl outline-none focus:border-emerald-500/50 appearance-none italic"
                                                    >
                                    ...
                                    <div className="p-10 bg-emerald-500/[0.03] rounded-[2.5rem] border border-emerald-500/10 flex items-center justify-between shadow-inner">
                                        <div className="flex items-center gap-6">
                                            <div className="w-16 h-16 bg-emerald-600/10 rounded-2xl flex items-center justify-center border border-emerald-500/20 shadow-lg">{getIcon(selectedGuest.mesa)}</div>
                                            <div>
                                                ...
                                                {!isEditingMesa ? (
                                                    ...
                                                    <div className="flex items-center gap-4 mt-2">
                                                        <input 
                                                            ...
                                                            className="bg-white/10 border-2 border-emerald-500 rounded-xl px-4 py-2 text-3xl font-black w-24 outline-none text-center shadow-2xl"
                                                        />
                                                        <button onClick={() => handleMesaUpdate(selectedGuest.id, tempMesa)} className="bg-emerald-600 px-6 py-4 rounded-xl font-black uppercase text-xs hover:bg-emerald-500">ACTUALIZAR</button>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        <button 
                                            ...
                                            className="w-20 h-20 bg-emerald-600 rounded-3xl flex items-center justify-center shadow-2xl active:scale-90 transition-all hover:scale-105"
                                        >
                            ...
                            <div className="p-10 lg:p-14 bg-white/[0.01] border-t border-white/5 flex flex-col sm:flex-row gap-8 items-center justify-between mt-auto">
                                <button onClick={() => { setSelectedGuest(null); setStatus('idle'); }} className="text-[12px] font-black text-gray-700 hover:text-white tracking-[0.5em] uppercase italic">Descarta_Nodo</button>
                                <button onClick={handleGrantAccess} className="w-full sm:w-auto px-24 py-10 bg-emerald-600 rounded-[2.5rem] text-white font-black uppercase tracking-[0.4em] shadow-[0_0_80px_#10b98150] hover:bg-emerald-500 active:scale-95 transition-all text-2xl flex items-center justify-center gap-6 group">
                                    ...
                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-[150] bg-[#05080f]/99 flex flex-col items-center justify-center p-12 text-center">
                                        {status === 'loading' && (
                                            <div className="space-y-12">
                                                <div className="relative">
                                                    ...
                                                    <div className="absolute inset-0 flex items-center justify-center"><Fingerprint className="w-16 h-16 text-emerald-500/20 animate-pulse" /></div>
                                                </div>
                                        ...
                                        {status === 'success' && (
                                            <div className="space-y-12">
                                                ...
                                                <p className="text-9xl font-black text-emerald-500 tracking-tighter">LISTO</p>
                                                <p className="text-2xl font-bold italic text-gray-500 uppercase tracking-widest mt-8">Acceso Validado Correctamente</p>
                                            </div>
                                        )}
                                        {status === 'error' && (
                                            <div className="space-y-12">
                                                <div className="w-32 h-32 bg-red-500/10 border-2 border-red-500/50 rounded-[3rem] flex items-center justify-center mx-auto"><WifiOff className="w-16 h-16 text-red-500" /></div>
                                                <p className="text-6xl font-black text-red-500 tracking-tighter uppercase">Fallo_de_Red</p>
                                                <div className="bg-red-500/5 p-8 rounded-3xl border border-red-500/10 max-w-md mx-auto">
                                                    <p className="text-sm font-bold text-gray-400 mb-6">El servidor (Stack Central) no responde. Por favor, verifica el estado de la API en el VPS.</p>
